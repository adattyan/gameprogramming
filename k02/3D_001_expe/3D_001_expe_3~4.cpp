...
void setLight(DataSet* data)
{
	...
	// 方向は(1,-1,-1)を向くライト。(実験③ライトの方向を適当に変更)
	light.Direction.x = 0;
	light.Direction.y = -1;
	light.Direction.z = 0;
	...
	// フィルライトは弱い白色に.(実験④ライトの色を適当に変更)
	light.Diffuse.r = 0.4f;
	light.Diffuse.g = 0.4f;
	light.Diffuse.b = 0.4f;
	// 方向は(-1, 1,-1)を向くライト。０番ライトと左右に逆方向(実験③ライトの方向を適当に変更)
	light.Direction.x = 1;
	light.Direction.y = 1;
	light.Direction.z = -1;
	...
	// バックライトは強めに。(実験④ライトの色を適当に変更)
	light.Diffuse.r = 0.8f;
	light.Diffuse.g = 0.8f;
	light.Diffuse.b = 0.8f;
	// 方向は(-1, 1,1)を向くライト。モデルを後ろから照らす。(実験③ライトの方向を適当に変更)
	light.Direction.x = 1;
	light.Direction.y = -1;
	light.Direction.z = 1;
